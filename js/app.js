...
async function callOpenRouter(messages, signal) {
    ...
    try {
        ...
        if (!config.apiEndpoints.openrouter || config.apiEndpoints.openrouter.length < 10) {
            throw new Error('API密钥未正确配置，请检查配置');
        }
...
function initializeConfig() {
    try {
        // 在CloudFlare Pages中，可以通过以下方式获取环境变量
        if (typeof OPENROUTER_API_KEY !== 'undefined') config.apiEndpoints.openrouter = OPENROUTER_API_KEY;
        
        // 加载认证信息
        if (typeof LOGIN_USERNAME !== 'undefined') config.auth.username = LOGIN_USERNAME;
        if (typeof LOGIN_PASSWORD !== 'undefined') config.auth.password = LOGIN_PASSWORD;
    } catch (error) {
        console.error('无法加载环境变量:', error);
    }
}
