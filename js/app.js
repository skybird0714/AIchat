...
function addEventListeners() {
    ...
    // 输入框键盘事件 - 修改键盘事件处理逻辑，确保Alt+S可以持续工作
    if (elements.userInput) {
        // 移除所有之前可能存在的键盘事件监听
        const oldKeydownListeners = elements.userInput._keydownListeners || [];
        oldKeydownListeners.forEach(listener => {
            elements.userInput.removeEventListener('keydown', listener);
        });
        ...
        const keydownHandler = function(e) {
            ...
            // 移除Enter键发送功能，保持默认换行行为
        };
        
        // 保存监听器的引用以便之后移除
        elements.userInput._keydownListeners = [keydownHandler];
        
        // 添加键盘事件监听
        elements.userInput.addEventListener('keydown', keydownHandler);
        
        console.log("已绑定Alt+S快捷键");
    }
...
function updateLoginButtonToAccountIcon() {
    ...
    loginButton.className = 'glass-button logout-button-style';
    
    // 更新点击事件为显示确认对话框
    loginButton.onclick = function() {
        utils.showConfirmDialog('确认退出', '确定要退出登录吗？', function() {
            logoutUser();
        });
        };
}
            ...
            sessionData.conversationHistory.forEach(msg => {
                if (msg.role === 'user') {
                    ...
                    formattedText = marked.parse(formattedText);
                        ...
                        console.error('历史消息Markdown渲染失败:', error);
                        try {
                            // 备用渲染方法
                            ...
                            formattedText = utils.escapeHtml(formattedText).replace(/\n/g, '<br>');
                        }
                    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (configData.OPENROUTER_API_KEY) {
                ...
                console.warn('未找到OpenRouter API密钥');
            }
            ...
            throw new Error(`配置API返回错误: ${response.status}`);
        }
    } catch (error) {
        ...
        // 显示错误提示
        setTimeout(() => {
            utils.showToast('API配置加载失败: ' + (error.message || '未知错误'), 'error');
            
            // 添加更明确的提示，帮助用户理解他们可以使用哪些模型
            setTimeout(() => {
                utils.showToast('仍然可以使用免费模型，但付费模型需要API密钥', 'info');
            }, 3000);
        }, 1000);
    }
}
